...
// People.Club (no icon.webp - use first screenshot)
...
// Triathlon Center (no icon.webp - use first screenshot)
...
function ProjectModal({ project, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-primary shadow-2xl"
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 z-10 rounded-full p-2 text-muted hover:text-text hover:bg-surface transition-colors"
        ...
        <div className="flex items-center gap-4 p-6 border-b border-border">
          <div className="shrink-0 w-20 h-20 rounded-2xl bg-surface flex items-center justify-center overflow-hidden">
            {project.image ? (
              <img
                src={project.image}
            ...
            <div className={`${project.image ? "hidden" : "flex"} w-full h-full items-center justify-center bg-linear-to-br from-accent/20 to-accent/5`}>
              <span className="text-3xl font-bold text-accent">{project.title.charAt(0)}</span>
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-xl font-semibold text-text mb-1">{project.title}</h3>
            <p className="text-sm text-accent font-medium">{project.summary}</p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {project.images && project.images.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-text uppercase tracking-wider mb-3">Screenshots</h4>
          ...
          {/* Description */}
          <div>
            <h4 className="text-sm font-semibold text-text uppercase tracking-wider mb-3">About</h4>
          ...
          {project.features && project.features.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-text uppercase tracking-wider mb-3">Key Features</h4>
              <ul className="grid gap-2">
                {project.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-muted">
                    <svg className="w-4 h-4 mt-0.5 text-accent shrink-0" fill="currentColor" viewBox="0 0 20 20">
          ...
          {/* Store Links */}
          <div>
            <h4 className="text-sm font-semibold text-text uppercase tracking-wider mb-3">Download</h4>
            <div className="flex flex-wrap gap-3">
              {project.appStoreUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-xl bg-surface border border-border px-4 py-3 text-sm font-medium text-text hover:bg-surface/80 hover:border-accent/50 transition-colors"
                >
                  <AppStoreIcon className="w-6 h-6" />
                  <div className="text-left">
                    <div className="text-[10px] text-muted uppercase">Download on the</div>
              ...
              {project.playStoreUrl && (
                ...
                >
                  ...
                  <div className="text-left">
                    <div className="text-[10px] text-muted uppercase">Get it on</div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((proj) => (
              <article
                ...
                className="group rounded-xl border border-border bg-surface/50 overflow-hidden transition-all hover:border-accent/50 hover:bg-surface hover:scale-[1.02] cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent/50 shadow-sm"
              >
                <div className="p-5">
                  <div className="flex items-start gap-4 mb-3">
                    <div className="shrink-0 w-14 h-14 rounded-xl bg-surface flex items-center justify-center overflow-hidden">
                      {proj.image ? (
                        <img
                          src={proj.image}
                          alt=""
                      ...
                      <div className={`${proj.image ? "hidden" : "flex"} w-full h-full items-center justify-center bg-linear-to-br from-accent/20 to-accent/5 [&:not(.hidden)]:flex`}>
                        <span className="text-xl font-bold text-accent">{proj.title.charAt(0)}</span>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-base text-text group-hover:text-accent transition-colors truncate">
                        {proj.title}
                      </h3>
                      <p className="text-xs text-muted mt-0.5">{proj.summary}</p>
                    </div>
                  </div>
